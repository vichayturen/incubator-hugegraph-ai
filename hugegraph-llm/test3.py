...
def tow_step_answer(question: str) -> Tuple[str]:
    ...
    llm = LLMs().get_llm()
    context = GraphRAG().match_keyword_to_id(topk_per_keyword=10).query_graph_for_rag().merge_dedup_rerank().run(
        query=question,
        keywords=[question]
    )
    graph_result = context['graph_result']
    prompt1 = template1.format(
        context_str="\n\n".join([f"{j + 1}. {text}" for j, text in enumerate(graph_result)]),
        query_str=question
    )
